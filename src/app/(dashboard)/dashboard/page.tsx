...
function MonthlyComparisonChart({ data }: { data: MonthlyTrendItem[] }) {
  ...
  return (
    <Card style={{ marginBottom:0, height:'100%', minHeight:330, display:'flex', flexDirection:'column' }}>
      <CardHead
      ...
      />
      <div style={{ padding:'0 16px 6px', flex:1, display:'flex', flexDirection:'column' }}>
        {chartData.length === 0 ? (
          <div style={{ fontSize:12, color:'#9ca3af' }}>Belum ada data historis untuk ditampilkan.</div>
        ) : (
          <>
            <div style={{ display:'flex', alignItems:'flex-end', gap:18, flex:1, minHeight:238, padding:'0 8px 0', overflowX:'auto' }}>
              {chartData.map(item => (
                <div key={`${item.month}-${item.year}`} style={{ minWidth:96, flex:1 }}>
                  <div style={{ height:214, display:'flex', alignItems:'flex-end', justifyContent:'center', gap:10, borderBottom:'1px solid #e5e7eb', paddingBottom:0 }}>
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'flex-end', height:'100%' }}>
                      ...
                      <div title={`Income ${fmt(item.income)}`} style={{ width:24, height:`${Math.max(8, (item.income / maxVal) * 182)}px`, background:'#16a34a', borderRadius:'7px 7px 0 0' }} />
                    </div>
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'flex-end', height:'100%' }}>
                      ...
                      <div title={`Outflow ${fmt(item.outflow)} · Expense ${fmt(item.expense)} · Saving ${fmt(item.saving)}`} style={{ width:24, height:`${Math.max(8, (item.outflow / maxVal) * 182)}px`, background:'#ef4444', borderRadius:'7px 7px 0 0' }} />
                    </div>
                  </div>
